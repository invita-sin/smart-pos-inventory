 ...
 *
 * Prasyarat:
 *   1. Isi SUPABASE_URL dan SUPABASE_SERVICE_ROLE_KEY di .env
 *      (Service role key: Supabase Dashboard > Project Settings > API > service_role key)
 *   2. Atau set sebagai environment variable:
...
if (!supabaseUrl || !serviceRoleKey) {
  console.error('Error: SUPABASE_URL dan SUPABASE_SERVICE_ROLE_KEY wajib diisi.')
  console.error('  Set di .env: SUPABASE_SERVICE_ROLE_KEY=your_key')
  console.error('  Atau export: export SUPABASE_SERVICE_ROLE_KEY=your_key')
  process.exit(1)
}
...
async function seed() {
  ...
  // 1. Create users via admin API (auto-confirmed)
  ...
  // 2. Categories
  const { data: cats } = await supabase.from('categories').upsert(seedCategories, { onConflict: 'name' }).select()
  console.log(`  ✅ ${cats?.length || 0} kategori`)

  // 3. Suppliers
  const { data: sups } = await supabase.from('suppliers').upsert(seedSuppliers, { onConflict: 'name' }).select()
  console.log(`  ✅ ${sups?.length || 0} supplier`)
  ...
  if (cats && sups) {
    ...
    // Hapus yang sudah ada berdasarkan SKU
    for (const prod of products) {
      const { error } = await supabase.from('products').upsert(prod, { onConflict: 'sku' })
      if (error) console.error(`  ❌ Gagal insert ${prod.sku}: ${error.message}`)
    }
