...
async function save() {
  if (editingCategory.value) {
    await categoryStore.update(editingCategory.value.id, form.value)
  } else {
...
async function deleteCategory() {
  if (deleteTarget.value) {
    await categoryStore.remove(deleteTarget.value.id)
...
<template>
  <div>
    ...
    <SkeletonLoader v-if="categoryStore.loading" variant="table" :cols="3" :rows="5" />
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-gray-200 dark:border-gray-700">
            ...
            <th class="text-left px-4 py-3 font-medium text-gray-500 dark:text-gray-400">Deskripsi</th>
            <th class="text-center px-4 py-3 font-medium text-gray-500 dark:text-gray-400">Aksi</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="c in categoryStore.categories" :key="c.id" class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50">
            ...
            <td class="px-4 py-3">
              <div class="flex items-center justify-center gap-2">
                <button @click="openEdit(c)" class="p-1.5 rounded-lg text-primary hover:bg-primary/10 transition-colors">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                </button>
                <button @click="confirmDelete(c)" class="p-1.5 rounded-lg text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 max-w-md w-full">
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <textarea v-model="form.description" rows="3" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary/50 outline-none"></textarea>
          </div>
        </div>
        <div class="flex gap-3 mt-6">
          ...
          <button @click="save" class="flex-1 py-2.5 rounded-lg bg-primary text-white font-medium">{{ editingCategory ? 'Simpan' : 'Tambah' }}</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 max-w-sm w-full">
        ...
        <div class="flex gap-3">
          ...
          <button @click="deleteCategory" class="flex-1 py-2.5 rounded-lg bg-red-500 text-white font-medium hover:bg-red-600">Hapus</button>
        </div>
      </div>
    </div>
  </div>
